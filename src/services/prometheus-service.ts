...
export class PrometheusService {
  ...
  public async getActiveVmCount(): Promise<number> {
    const response = (await this.queryPrometheus(
      'count(openstack_nova_server_status{status="ACTIVE"})',
  ...
  public async getShutoffVmCount(): Promise<number> {
    const response = (await this.queryPrometheus(
      'count(openstack_nova_server_status{status=~"SHUTOFF|SUSPENDED"})',
    )) as PrometheusResponse;
    return this.getCount(response);
  }
  ...
  public async getElseVmCount(): Promise<number> {
    const response = (await this.queryPrometheus(
      'count(openstack_nova_server_status{status!~"ACTIVE|SHUTOFF|SUSPENDED"})',
    )) as PrometheusResponse;
    return this.getCount(response);
  }
  ...
  public async getShutoffVmLookup(): Promise<PrometheusResponse> {
    return (await this.queryPrometheus(
      'openstack_nova_server_status{status=~"SHUTOFF|SUSPENDED"}',
    )) as PrometheusResponse;
  }

  public async getElseVmLookup(): Promise<PrometheusResponse> {
    return (await this.queryPrometheus(
      'openstack_nova_server_status{status!~"ACTIVE|SHUTOFF|SUSPENDED"}',
    )) as PrometheusResponse;
  }

  public async getVcpuTotalCount(): Promise<PrometheusResponse> {
    return (await this.queryPrometheus(
      'sum(openstack_placement_resource_total{resourcetype="VCPU"}) * avg(openstack_placement_resource_allocation_ratio{resourcetype="VCPU"})',
    )) as PrometheusResponse;
  }
  ...
  public async getMemoryTotalVolume(): Promise<PrometheusResponse> {
    return (await this.queryPrometheus(
      'sum(openstack_placement_resource_total{resourcetype="MEMORY_MB"}) * avg(openstack_placement_resource_allocation_ratio{resourcetype="MEMORY_MB"})',
    )) as PrometheusResponse;
  }
